...
async def get_current_user_token(authorization: Optional[str] = Header(None)):
    """
    헤더에서 'Bearer {username}' 형태의 토큰을 추출하여 유저 정보를 반환합니다.
    (간단한 구현을 위해 username을 토큰으로 사용합니다)
    """
    ...
    try:
        ...
        # 실제 운영 환경에서는 여기서 JWT 검증을 해야 합니다.
        # 현재는 토큰(token)이 곧 유저네임(username)이라고 가정합니다.
...
async def login(user: AuthModel):
    ...
    # [수정] access_token 반환 (프론트엔드 연동용)
    return {
        "access_token": row["username"], # 임시로 username을 토큰으로 사용
        "token_type": "bearer",
