...
async function loadComponents() {
    ...
    // 초기화 함수 호출 (game.js, api.js 등에 정의됨)
...
function renderDiceGrid(list) {
    ...
    list.forEach(dice => {
        ...
        // aspect-square w-full + shrink-0 로 찌그러짐 방지
        const cardHtml = `
    ...
    if (dice.class_level >= 20) { // 만렙 처리
        ...
        if (dice.class_level === 0) {
            ...
            if (hasEnoughCards && hasEnoughGold) {
                ...
                btnColorClass = "bg-green-600 hover:bg-green-700"; // 초록색 테마
                progColorClass = "bg-green-500";
            } else {
                currentViewMode = 'class'; // 불가능해도 미리보기 표시
...
function playSingleSummonAnimation(diceData, isNew) {
    ...
    // 결과 아이콘 미리 생성 (w-32 h-32, text-7xl)
    let realIconHtml = renderDiceIcon(diceData, "w-32 h-32");
    realIconHtml = realIconHtml.replace("text-4xl", "text-7xl"); 

    // [3D Flip 구조]
    const flipHtml = `
        <div class="flip-container summon-drop">
            <div id="dice-flipper" class="flipper">
                <div class="flip-face flip-front">
                    ${realIconHtml}
                    ${isNew ? `<div class="absolute -top-4 -right-4 bg-red-500 text-white text-sm font-bold px-2 py-1 rounded-full shadow-lg border-2 border-white new-badge-pop z-20">NEW!</div>` : ''}
                </div>
                
                <div class="flip-face flip-back" style="--glow-color: ${config.color}">
                    <i class="${config.icon} text-6xl ${config.tailwind} opacity-50"></i>
                </div>
            </div>
        </div>
    `;
    
    container.innerHTML = flipHtml;
    
    // [1단계] 하강 후 대기
    const flipper = document.getElementById('dice-flipper');
    const backFace = flipper.querySelector('.flip-back');
    const containerDiv = container.querySelector('.flip-container');
    ...
    setTimeout(() => {
        ...
        // 등급 아이콘 진하게 & 글로우
        backFace.classList.add('summon-waiting');
        backFace.innerHTML = `<i class="${config.icon} text-7xl ${config.tailwind}"></i>`;
        
        // 클릭 이벤트 연결 (뒤집기)
        containerDiv.onclick = () => revealDiceFlip(flipper, diceData, config);
        
    }, 600);
}

function revealDiceFlip(flipper, diceData, config) {
    const containerDiv = flipper.parentElement;
    containerDiv.onclick = null; // 중복 클릭 방지
    
    const backFace = flipper.querySelector('.flip-back');
    backFace.classList.remove('summon-waiting');
    
    // [2단계] 파동 이펙트
    const ripple = document.createElement('div');
    ripple.className = 'ripple-effect';
    ripple.style.setProperty('--glow-color', config.color);
    containerDiv.appendChild(ripple);
    ...
    setTimeout(() => {
        ...
        diceName.style.color = config.color; // 이름 색상
