    ...
    try {
        ...
        if (res.ok) {
            ...
            if(tbody) {
                ...
                const map = { 'pending_signup': '회원가입요청', 'pending_reset': '비번변경요청', 'pending_deletion': '탈퇴요청' };
                users.forEach(u => {
    ...
    listEl.innerHTML = users.map(u => `
        <div class="user-item">
            <div style="display:flex; align-items:center; gap:10px;">
                <input type="checkbox" class="user-check" value="${u.username}" onchange="updateSelection('${u.username}', this.checked)" ${selectedUsers.has(u.username) ? 'checked' : ''} style="width:18px; height:18px; margin:0;">
                <div><span style="font-weight:bold;">${u.nickname}</span><span style="font-size:12px; color:#888;">(${u.username})</span></div>
            </div>
        </div>`).join('');
}
...
function updateSelection(username, isChecked) { if(isChecked) selectedUsers.add(username); else selectedUsers.delete(username); }
...
async function sendAdminMail() {
    ...
    try {
        ...
        if(res.ok) { alert("발송되었습니다!"); selectedUsers.clear(); document.getElementById('mail-title').value=""; document.getElementById('mail-content').value=""; document.getElementById('mail-receiver-display').value=""; }
        else { alert("발송 실패"); }
...
async function fetchMailHistory() {
    try {
        ...
        container.innerHTML = list.map(item => {
            ...
            return `<div class="history-item"><div style="display:flex; justify-content:space-between;"><strong>${item.title}</strong>${statusTag}</div><p style="font-size:12px; margin:5px 0;">${item.content}</p><div style="font-size:11px; color:#888;">수신: ${item.receiver_count}명 | 작성: ${item.created_at.substring(0,16)} ${cancelBtn}</div></div>`;
        }).join('');
    } catch(err) {}
}
...
// [수정] renderDB: 드롭다운(Select)이 있으면 옵션 추가, 없으면 버튼 생성
function renderDB(tables) {
    const area = document.getElementById('db-content-area');
    const selectBox = document.getElementById('table-select'); // 드롭다운
    const linksDiv = document.getElementById('table-links'); // 기존 버튼 영역
    ...
    // 드롭다운 초기화
    if(selectBox) {
        selectBox.innerHTML = '<option value="">테이블을 선택하세요</option>';
        tables.forEach(t => {
    ...
    // 기존 버튼 방식 (admin.html 등에서 사용 시 대비)
    else if(linksDiv) {
        linksDiv.innerHTML = "";
        tables.forEach(t => {
            const btn = document.createElement('button');
            btn.innerText = t.name;
            btn.onclick = () => showTable(t.name);
            linksDiv.appendChild(btn);
        });
    }

    // 각 테이블의 데이터 섹션 생성 (숨김 상태로 모두 생성해둠)
    tables.forEach(t => {
...
function showTable(tableName) {
    ...
    if(tableName) {
        const target = document.getElementById(`section-${tableName}`);
...
window.showTable = showTable; // 전역 등록 추가